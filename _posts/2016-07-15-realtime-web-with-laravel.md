...
**언제 어떻게 쓸 수 있나?** 새로운 댓글이 작성되면 같은 댓글 목록을 보고 있는 사용자에게 새 댓글을 알리거나, 현재 서비스에 현재 접속된 모든 사용자에게 푸시 알림을 보내는 등으로 사용할 수 있다. 

이 포스트는 데모 프로젝트를 복붙으로 따라해보기 위한 명령 및 코드 목록을 담고 있다. 작동 원리는 읽지 말고, 전체 동작을 한번 확인할 것을 권장한다. 
...
라라벨의 `pub`라우팅을 방문하면, Redis 서버에 `whik` 채널로 JSON 직렬화된 레코드를 쓰도록 작성했다.
...
Redis에 새 레코드가 발행되면, 서버는 `whik:App\\Events\\NewUserCreated` 채널로 접속된 모든 Socket.io 클라이언트에게 메시지를 브로드캐스트한다. 
...
수정할 코드는 없다. `feature/broadcast` 태그로 체크아웃한다.
...
변경 내용은 [커밋로그](https://github.com/appkr/whik/commit/57365503be757253a0830590d74fe9e8ea855b0b)에서 확인한다.

### 4.3. 작동 원리

라라벨은 프레임워크 어디서든 도메인 이벤트를 발행할 수 있다. 여기서는 이벤트 클래스(이벤트 채널, Data Transfer Object)를 이용해서 이벤트를 던지는 것으로 구현했다.

#### 4.3.1. 이벤트 클래스

이벤트 클래스는 아티즌 콘솔로 만든다(`$ php artisan make:event NewUserCreated`). 브로드캐스트 기능을 쓰려면, 이벤트 클래스에서 `ShouldBroadcast` 인터페이스를 구현하면 된다. 인터페이스의 메서드는 `broadcastOn()` 하나이고, 브로드캐스트할 채널의 이름만 반환하면 된다.
...
이제 `NewUserCreated` 이벤트가 발생하면, 라라벨은 `public`으로 선언된 이벤트 데이터(클래스 프로퍼티)를 자동으로 JSON 직렬화하고, `config/broadcasting.php`에 정의한 드라이버로 브로드캐스팅한다. 
