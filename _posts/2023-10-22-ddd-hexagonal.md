...
지금까지 사례는 부릉이었고, 이번 사례는 플렉스입니다. 왼쪽이 `Service`, 오른쪽이 `AggregateRoot Entity`(이하 "모델"이란 용어와 혼용합니다) 입니다. `Service`는 다음과 같은 일을 합니다. 모든 기업이 급여 지급일의 다음 달 10일까지 국세청에 제출해야 하는 "원천징수이행상황신고서"라는 서식을 만드는 유스케이스입니다. <small class="text-muted">클래스 이름이 컨텍스트를 포함하고 있어서 유스케이스의 이름을 `create`로 짧게 지었습니다</small>

- **27줄**: 데이터베이스 트랜잭션을 관리합니다; `@Transactional` 애너테이션을 애플리케이션 안에서 사용하는 부분에 대한 의견이 갈릴 수 있습니다. 다시 한번 강조하지만 저는 실용주의자입니다, 갑자기 Quarkus 프레임웍으로 갈아탈 가능성은 없다고 봅니다
...
> 응용 서비스는 트랜잭션 처리도 담당한다. 응용 서비스는 도메인의 상태 변경을 트랝개션으로 처리해야 한다. (후략) [🔗](https://www.yes24.com/Product/Goods/108431347) 
